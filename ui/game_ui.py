...
def draw_game(screen, game):
    ...
    if fairway_points and len(fairway_points) >= 2:
        ...
        for i in range(len(fairway_points)):
            ...
            nx = -dy / length
            ny = dx / length
            left_point = (fairway_points[i][0] + nx * half_width, fairway_points[i][1] + ny * half_width)
            right_point = (fairway_points[i][0] - nx * half_width, fairway_points[i][1] - ny * half_width)
    ...
    sorted_obs = sorted(hole["obstacles"], key=lambda o: (z_map.get(o["type"], 2), o["pos"][1]))
        ...
        if obs["type"] == "water":
            ...
            for i in range(6):
                ...
                if sz > 0:
                    ...
                    pygame.draw.ellipse(wave_surf, (255, 255, 255, alpha), (0, 0, sz*2, sz))
                    screen_pos = camera.world_to_screen((bx, by))
                    screen.blit(wave_surf, (screen_pos[0] - sz, screen_pos[1] - sz//2))
    ...
    for i, ball in enumerate(game.balls):
        if ball.visible and not ball.in_hole:
            ...
            shadow_world = (ball.pos[0] - 12 + shadow_offset, ball.pos[1] - 6 + shadow_offset)
    ...
    if (game.shot_system.is_aiming and not game.shot_system.selecting_angle
            and ball_active.visible and not ball_active.moving):
        ...
        if dist > 5:
            ...
            line_len = power * 0.3
            end_world = (ball_pos[0] + direction[0] * line_len,
...
def draw_angle_selector(screen, game):
    ...
    pygame.draw.circle(screen, config.WHITE, (cx, int(indicator_y)), int(6 * camera.zoom), 1)

    angle_text = game.small_font.render(f"Угол: {int(game.shot_system.angle_value)}°", True, config.WHITE)
    screen.blit(angle_text, (cx - 40, cy - radius - 20))
    prompt = game.small_font.render("Двигайте мышь вниз для подъёма", True, config.WHITE)
