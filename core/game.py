...
class GolfGame:
    ...
    def __init__(self, screen):
        ...
        color_files = ["BALLRED.png", "BALLBLUE.png", "BALLWELLOY.png", "BALLPERPULE.png"]
        ...
        for fname in color_files:
            ...
            if img is None:
                ...
                color = config.PLAYER_COLORS[idx] if idx < len(config.PLAYER_COLORS) else (255,255,255)
    ...
    def start_game(self):
        ...
        self.message = ""
        self.particle_system.init_for_weather(self.field_type, self.weather, self.holes_data[0]["wind"])
    ...
    def reset_hole(self):
        ...
        self.particle_system.init_for_weather(self.field_type, self.weather, hole["wind"])
    ...
    def update(self, dt):
        if self.state == GameState.GAME:
            msgs = update_physics(self.balls, self.holes_data[self.current_hole], dt,
                                  self.session.strokes, self.session.players_finished,
            ...
            self.ball_moving = any(b.visible and (b.moving or b.in_flight) for b in self.balls)
    ...
    def draw(self):
        if self.state in (GameState.MENU_PLAYERS, GameState.MENU_HOLES,
                          GameState.MENU_FIELD, GameState.MENU_WEATHER):
            draw_menu(self.screen, self.state, self.menu_buttons, self.message,
                      self.font_large, self.font, self.small_font)
        elif self.state == GameState.MENU_RECORDS:
            content_height, corrected_offset = draw_records_screen(
                self.screen, self.font_large, self.font, self.small_font,
                self.menu_buttons, self.message, self.records_scroll_offset)
    ...
    def draw_game_over(self):
        ...
        draw_game_over_background(self.screen, self.font_large, self.font, self.small_font,
                                  totals, self.num_players)
        if self.record_message:
            record_surf = self.font.render(self.record_message, True, config.YELLOW)
            record_rect = record_surf.get_rect(center=(config.SCREEN_WIDTH//2, config.SCREEN_HEIGHT-200))
    ...
    def advance_hole(self):
        ...
        if self.current_hole >= self.num_holes:
            ...
            if records.is_new_record(self.field_type, self.weather, self.num_holes, min_strokes):
                self.record_message = f"НОВЫЙ РЕКОРД! {winner_name}: {min_strokes} ударов ({self.num_holes} лунки)"
                records.update_record(self.field_type, self.weather, self.num_holes, min_strokes, winner_name)
            else:
                current_record = records.get_record(self.field_type, self.weather, self.num_holes)
                if current_record:
    ...
    def setup_menu(self):
        ...
        if self.state == GameState.MENU_PLAYERS:
            ...
            for i, n in enumerate([2,3,4,5]):
                ...
                self.menu_buttons.append(Button((config.SCREEN_WIDTH//2-200, 200+i*80, 400,70),
                                                f"{n} игрока", colors[i], hovers[i]))
            self.menu_buttons.append(Button((config.SCREEN_WIDTH//2-150, 550, 300,60),
                                            "Рекорды", config.BLUE, (100,100,255)))
            self.menu_buttons.append(Button((config.SCREEN_WIDTH//2-150, 630, 300,60),
                                            "Сбросить рекорды", config.RED, (255,100,100)))
        elif self.state == GameState.MENU_RECORDS:
            self.message = ""
            self.menu_buttons.append(Button((config.SCREEN_WIDTH//2-100, config.SCREEN_HEIGHT-70, 200,50),
                                            "Назад", config.GRAY, (180,180,180)))
            self.records_scroll_offset = 0
        elif self.state == GameState.MENU_HOLES:
            self.message = "Выберите количество лунок"
            for i, h in enumerate([3,6,9]):
                self.menu_buttons.append(Button((config.SCREEN_WIDTH//2-200, 200+i*100, 400,70),
                                                f"{h} лунок", config.YELLOW, (255,200,0)))
            ...
            for i,(key,name) in enumerate(fields):
                color = config.DARK_GREEN if key=="forest" else config.SAND_COLOR if key=="desert" else config.SNOW_WHITE
                hover = (0,150,0) if key=="forest" else (210,180,140) if key=="desert" else (230,230,230)
                txt_col = config.WHITE if key=="forest" else config.BLACK
                self.menu_buttons.append(Button((config.SCREEN_WIDTH//2-200, 200+i*100, 400,70),
                                                name, color, hover, txt_col))
        elif self.state == GameState.MENU_WEATHER:
            self.message = "Выберите погоду"
            w_opts = [("sunny","Солнечно",config.YELLOW), ("rain","Дождь",config.GRAY), ("windy","Ветрено",config.LIGHT_BLUE)]
            for i,(key,name,col) in enumerate(w_opts):
                hover = (255,200,0) if key=="sunny" else (100,100,100) if key=="rain" else (135,206,250)
                self.menu_buttons.append(Button((config.SCREEN_WIDTH//2-200, 200+i*100, 400,70),
                                                name, col, hover, config.BLACK))
    ...
    def handle_event(self, event):
        if self.shot_system.selecting_angle:
            if event.type == pygame.MOUSEMOTION:
                ...
                self.shot_system.update_angle(world_mouse[1], ball.pos[1], self.camera.zoom)
        ...
        if event.type == pygame.MOUSEWHEEL:
            ...
            if event.button == 1:
                if self.state in (GameState.MENU_PLAYERS, GameState.MENU_HOLES,
                                  GameState.MENU_FIELD, GameState.MENU_WEATHER, GameState.MENU_RECORDS):
                    ...
                    if not self.shot_system.is_aiming:
                        ball = self.balls[self.session.active_player]
                        world_mouse = self.camera.screen_to_world(event.pos)
                        if (not self.ball_moving and ball.visible and not ball.in_hole
                                and math.hypot(world_mouse[0] - ball.pos[0], world_mouse[1] - ball.pos[1]) < 40):
    ...
    def process_menu_click(self, text):
        if self.state == GameState.MENU_PLAYERS:
            if "2" in text: self.num_players = 2
            elif "3" in text: self.num_players = 3
            elif "4" in text: self.num_players = 4
            elif "5" in text: self.num_players = 5
                ...
                self.setup_menu()
                return
            self.state = GameState.MENU_HOLES; self.setup_menu()
        elif self.state == GameState.MENU_RECORDS:
            if "Назад" in text:
                self.state = GameState.MENU_PLAYERS
                self.setup_menu()
        elif self.state == GameState.MENU_HOLES:
            self.num_holes = int(text.split()[0]); self.state = GameState.MENU_FIELD; self.setup_menu()
        elif self.state == GameState.MENU_FIELD:
            if text == "Лес": self.field_type = "forest"
            elif text == "Пустыня": self.field_type = "desert"
            elif text == "Снег": self.field_type = "snow"
            self.state = GameState.MENU_WEATHER; self.setup_menu()
        elif self.state == GameState.MENU_WEATHER:
            if text == "Солнечно": self.weather = "sunny"
            elif text == "Дождь": self.weather = "rain"
            elif text == "Ветрено": self.weather = "windy"
            self.start_game()
